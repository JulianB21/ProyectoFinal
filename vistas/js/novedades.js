...
$('.tablaArticulos tbody').on('click', 'button.btnAgregarArticulo', function() {
    ...
    $.ajax({
        ...
        success: function(respuesta) {
            ...
            $(".nuevoArticulo").append('<div class="row" style="padding: 5px 15px">' + '<div class="col-xs-4" style="padding-right:0px">' + '<div class="input-group">' + '<span class="input-group-addon"><button type="button" class="btn btn-danger quitarNovedad btn-xs" idArticulo="' + idArticulo + '"><i class="fa fa-times"></i></button></span>' + '<input type="text" class="form-control " id="agregarArticulo" name="agregarArticulo" value="' + nombreArticulo + '" required readonly>' + '</div>' + '</div>' + '<div class="form-group col-xs-4"  style="padding-left:5px; padding-right: 0px">' + '<div class="input-group">' + '<span class="input-group-addon"><i class="fa fa-th"></i></span>' + '<select class="form-control" name="tipoNovedadArticulo" id="tipoNovedadArticulo">' + '<option value="">Tipo</option>' + '<option value="DAÑADO">DAÑADO</option>' + '<option value="PERDIDO">PERDIDO</option>' + '</select>' + '</div>' + '</div> ' + '<div class="col-xs-4" style="padding-left:5px">' + '<div class="input-group">' + '<input type="text" class="form-control " id="nuevaDescripcion" name="nuevaDescripcion" placeholder="Descripción" required>' + '</div>' + '</div>' + '</div>')
        }
    });
});
// QUITAR NOVEVDAD
